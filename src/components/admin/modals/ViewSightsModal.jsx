...
import { DeleteCitySights, RefreshCitySights } from '../../../server-communicator/ServerCommunicator';

function ViewSightsModal({ show, onHide, data, showToast, toastHeader, toastBody, refresh }) {
    ...
    const handleRefresh = async () => {
        const response = await RefreshCitySights(data?.Lat, data?.Lon, token)
        ...
        if (response.error) {
            ...
            toastHeader('Success')
            toastBody('Successfully refreshed City Sights for ' + data?.CityName)
        }
        onHide()
        showToast()
        refresh()
    ...
    const handleDelete = async () => {
        const response = await DeleteCitySights(data?.Id, token);
        if (response.error) {
            toastHeader('Failed')
            toastBody(response.error)
        } else {
            toastHeader('Success')
            toastBody('Successfully deleted City Sights for ' + data?.CityName)
        }
...
export default ViewSightsModal;
